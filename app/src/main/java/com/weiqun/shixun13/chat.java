    ...
    public void onClick(View v) {
        ...
        UtilsOk.httpPostUtils_Reg_lt("http://123.207.85.214/chat/chat1.php", user, chat, new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                Looper.prepare();
                Toast.makeText(chat.this, "数据请求失败", Toast.LENGTH_SHORT).show();
                Looper.loop();
            }
    ...
    private void SHOWData(final List<userInfo> userInfos){
        runOnUiThread(new Runnable() {//UI线程的更新
            ...
            public void run() {
                ...
                lv.setText("用户名:"+userInfos.get(5).getName()+"   "+""+userInfos.get(5).getTime()+"\n");
                lv.append("内容:"+userInfos.get(5).getChat()+"\n");
                lv.append("                \n");
                lv.append("用户名:"+userInfos.get(4).getName()+"   "+""+userInfos.get(4).getTime()+"\n");
                lv.append("内容:"+userInfos.get(4).getChat()+"\n");
                lv.append("                \n");
                lv.append("用户名:"+userInfos.get(3).getName()+"   "+""+userInfos.get(3).getTime()+"\n");
                lv.append("内容:"+userInfos.get(3).getChat()+"\n");
                lv.append("                \n");
                lv.append("用户名:"+userInfos.get(2).getName()+"   "+""+userInfos.get(2).getTime()+"\n");
                lv.append("内容:"+userInfos.get(2).getChat()+"\n");
                lv.append("                \n");
                lv.append("用户名:"+userInfos.get(1).getName()+"   "+""+userInfos.get(1).getTime()+"\n");
                lv.append("内容:"+userInfos.get(1).getChat()+"\n");
                lv.append("                 \n");
                lv.append("用户名:"+userInfos.get(0).getName()+"   "+""+userInfos.get(0).getTime()+"\n");
                lv.append("内容:"+userInfos.get(0).getChat()+"\n");
//                基本适配器
